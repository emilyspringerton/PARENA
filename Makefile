...
# test-g711 -- real end-to-end verification for stdlib/sip/g711.prn (CarePyre SIP Phone Phase 4,
# the audio codec half): mu-law/A-law encode+decode against hand-derived exact values from the
# real canonical Sun/CCITT reference this module directly ports.
test-g711: build
	./parena build stdlib/sip/g711.prn -o tests/test_g711_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_g711.c \
		runtime/parena_runtime.c -o /tmp/test_g711_bin -lm
	/tmp/test_g711_bin
...
# test-net-udp -- real end-to-end verification of stdlib/net/udp.prn (2026-09-11 rewrite -- see
# that file's own header comment for the real bug list this closes). Real live sockets, not a
# mock: binds two real UDP sockets, sends a real datagram, receives it, asserts the payload and
# the sender's own reported address/port both come back correct.
test-net-udp: build
	./parena build stdlib/string.prn stdlib/net/udp.prn -o tests/test_net_udp_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_net_udp.c \
...
# test-net-unixsocket -- real end-to-end verification for stdlib/net/unixsocket.prn (S498,
# founder real-time: "double down on all the unix socket stuff and raw socket stuff"). Unlike
# rawsocket/l2socket, this needs no CAP_NET_RAW/root -- every assertion is a real, live,
# unprivileged round trip.
test-net-unixsocket: build
	./parena build stdlib/string.prn stdlib/net/unixsocket.prn -o tests/test_net_unixsocket_gen.c
...
# test-serial -- real end-to-end verification for stdlib/hw/serial.prn
# (docs/UART_SERIAL_NORTHSTAR.md, kanban cards HW-001/HW-003). This box
# has no physical USB-serial device (that doc's own Phase 2 already
# names this as a real, honest limitation), so this exercises the real
# open()+termios-configure()+read()+write()+close() sequence against a
...
# test-spi -- real end-to-end verification for stdlib/hw/spi.prn, scoped
# to what's genuinely testable in this sandbox (no real spidev controller
...
# test-bytes -- real end-to-end verification for stdlib/bytes.prn
# (docs/BYTES_NORTHSTAR.md), PARENA's real second core-language base
# type alongside String, closing the embedded-NUL-byte gap test-spi/
# test-serial/test-i2c all independently found and named the same day.
test-bytes: build
	./parena build stdlib/bytes.prn -o tests/test_bytes_gen.c
...
# test-pentest-x509 -- real end-to-end verification for stdlib/pentest/x509.prn (PARENA
# cybersecurity-primitives thread, 2026-09-07): a genuine openssl-generated DER certificate's own
# real version/serialNumber, extracted via the ldap/ber long-form BER extension.
test-pentest-x509: build
	./parena build stdlib/string.prn stdlib/net/wire.prn stdlib/ldap/ber.prn stdlib/pentest/x509.prn \
		-o tests/test_pentest_x509_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_pentest_x509.c \
		runtime/parena_runtime.c -o /tmp/test_pentest_x509_bin -lm
	/tmp/test_pentest_x509_bin

# test-net-rawsocket -- real end-to-end verification for stdlib/net/rawsocket.prn (PARENA
# cybersecurity-primitives thread, 2026-09-07 follow-up: "Raw Socket Protocol Overrides
# (IP_HDRINCL)"). Real, honest, environment-dependent: this sandbox has no real CAP_NET_RAW/root,
# so the real, deterministic PermissionDenied path is what actually runs here.
test-net-rawsocket: build
	./parena build stdlib/string.prn stdlib/net/rawsocket.prn -o tests/test_net_rawsocket_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_net_rawsocket.c \
		runtime/parena_runtime.c -o /tmp/test_net_rawsocket_bin -lm
	/tmp/test_net_rawsocket_bin

# test-pentest-procmaps -- real end-to-end verification for stdlib/pentest/procmaps.prn (PARENA
# cybersecurity-primitives thread, 2026-09-07 follow-up: "Structured Binary System File Parsers
# ... Linux /proc/ maps"). Parses this test binary's OWN real, live /proc/self/maps.
test-pentest-procmaps: build
	./parena build stdlib/string.prn stdlib/array.prn stdlib/io.prn stdlib/pentest/procmaps.prn \
		-o tests/test_pentest_procmaps_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_pentest_procmaps.c \
		runtime/parena_runtime.c -o /tmp/test_pentest_procmaps_bin -lm
	/tmp/test_pentest_procmaps_bin

# test-set -- real end-to-end verification for stdlib/set.prn (PARENA data-analysis-primitives
# thread, 2026-09-07 follow-up: "Set Theory and Set Convergence Primitives ... intersections,
# unions, and set differences"). Real IOC-vs-observed-connections intersect/union/difference.
test-set: build
	./parena build stdlib/string.prn stdlib/array.prn stdlib/set.prn -o tests/test_set_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_set.c \
		runtime/parena_runtime.c -o /tmp/test_set_bin -lm
	/tmp/test_set_bin

# test-io-mmap -- real end-to-end verification for stdlib/io/mmap.prn (PARENA
# cybersecurity-primitives thread, 2026-09-07 follow-up: "Raw Disk / Memory-Mapped File
# Primitives (mmap)"). Real, live mmap of an actual file, zero-copy access verified.
test-io-mmap: build
	./parena build stdlib/string.prn stdlib/io/mmap.prn -o tests/test_io_mmap_gen.c
	$(CC) -std=c99 -Wall -Wextra -pedantic -Werror -I runtime -I tests tests/test_io_mmap.c \
		runtime/parena_runtime.c -o /tmp/test_io_mmap_bin -lm
	/tmp/test_io_mmap_bin

# test-linalg-sparse -- real end-to-end verification for stdlib/linalg.prn's new CSR sparse-
# matrix primitives (PARENA data-analysis-primitives thread, 2026-09-07/08 follow-up: "Sparse
# Matrix Primitives... CSR formats"). Real 4x4 sparse matrix from COO triplets, real matvec.
test-linalg-matmul: build
	./parena build stdlib/array.prn stdlib/linalg.prn -o tests/test_linalg_matmul_gen.c
