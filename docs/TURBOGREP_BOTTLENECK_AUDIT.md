...
| G4 | Forward declarations in `src/emit.c` (`emit_forward_decls`, ~line 5394-5414) are deliberately empty-parens/K&R-style (`ReturnType mangled_name();`) — documented, deliberate tradeoff to avoid duplicating `emit_defn`'s parameter-type-resolution logic | **Fixed 2026-08-25** (PARENA `99ef91a`, Apple #15728). Empty-parens forward decls meant gcc COULD NOT type-check call-site arguments against the real function signature — this let `match-star`'s dispatch call in `match-node` pass a `PatternNode` struct BY VALUE where `match_star`'s real definition expects `PatternNode *` compile with zero warnings and segfault at runtime (`./turbogrep 'a*b' file` → exit 139), fixed locally in `9a622f5`. New `resolve_param_prototype_type()`/`build_defn_prototype()` mirror `emit_defn`'s own parameter-shape matching to emit a fully-typed prototype (`RetType name(T1, T2, ...);`) whenever every parameter's shape resolves, falling back to empty-parens only for shapes the mirror doesn't recognize — never a regression. Verified with a standalone repro that this turns the exact Star bug class into a hard gcc compile ERROR (not just a warning, no `-Wall` needed). All 336 compiler tests pass; turbogrep rebuilt and re-verified byte-identical against real grep across the 951-file corpus. |
...
trust": **C11 (MatchBudget, unverified) > C10 (binary files)**. C1/C2/C3/C4
(CharClass/Anchor/Plus/Optional) and G4 (empty-parens forward decls) are now all fixed. Highest
real remaining perf lever: **P3/P4** (general matcher + multi-literal Alt), likely bigger than
P5/P6/P7 combined for real-world grep usage. G1/G2 remain open, compiler-level, and would each
unblock multiple stdlib gaps at once rather than one at a time.
