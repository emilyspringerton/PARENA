 ...
 *
 * One, real, minimal adaptation from the pristine upstream in this file specifically: `#define
 * _GNU_SOURCE` moved to the very first line, before any #include. glibc's feature-test macros are
 * only honored based on state at the FIRST system header included in a translation unit --
 * upstream's own placement (after stddef.h/stdint.h/stdlib.h) left `syscall()` (used just below)
 * with no visible prototype, a real (harmless on x86-64's own varargs ABI, but sloppy) compiler
 * warning found while integrating this into BIG_O's own build. Fixed properly, not suppressed,
 * same discipline every other real warning found in this monorepo already gets -- the actual
 * getrandom()/CryptGenRandom()/`/dev/urandom` logic below is completely unchanged.
 */
#define _GNU_SOURCE
#include <stddef.h>
#include <stdint.h>
