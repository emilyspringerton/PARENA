...
static inline __attribute__((unused)) Registry Registry_new(Vec docs, int current_index) {
    Registry v;
    v.docs = docs;
    v.current_index = current_index;
    return v;
}
...
Registry new_registry(Arena *);
Document vec_document_at(Vec *, int);
Registry open_document(Registry, int, Arena *);
Registry switch_document(Registry, int);
Document current_document(Registry *);
int document_count(Registry *);
...
static inline Document *Document_box(Arena *dest, Document v) {
    Document *p = (Document *)arena_alloc(dest, sizeof(Document));
    *p = v;
    return p;
}
...
Registry new_registry(Arena *dest __attribute__((unused))) {
    return Registry_new(vec_new(dest), -1);
}

Document vec_document_at(Vec * v __attribute__((unused)), int idx __attribute__((unused))) {
    return (*(Document *)vec_get(v, idx));
}

Registry open_document(Registry reg __attribute__((unused)), int now_epoch __attribute__((unused)), Arena *dest __attribute__((unused))) {
    Document new_doc __attribute__((unused)) = new_document(now_epoch, dest);
    Vec docs __attribute__((unused)) = (reg).docs;
    (void)(vec_push_(&(docs), Document_box(dest, new_doc)));
    return Registry_new(docs, (vec_len(&(docs)) - 1));
}

Registry switch_document(Registry reg __attribute__((unused)), int index __attribute__((unused))) {
    if (((index >= 0) && (index < vec_len(&((reg).docs))))) {
    return Registry_new((reg).docs, index);
    } else {
    return reg;
    }
}

Document current_document(Registry * reg __attribute__((unused))) {
    return vec_document_at(&((reg)->docs), (reg)->current_index);
}

int document_count(Registry * reg __attribute__((unused))) {
    return vec_len(&((reg)->docs));
}
